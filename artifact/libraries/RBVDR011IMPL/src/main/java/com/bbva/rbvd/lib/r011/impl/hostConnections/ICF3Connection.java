...
public class ICF3Connection extends AbstractLibrary {
    ...
    public EntityOutPolicyCancellationDTO executeICF3Transaction(InputParametersPolicyCancellationDTO input,
        ...
        LOGGER.info("***** ICF3Connection - executeICF3Transaction - Start");
        ...
        LOGGER.info("***** ICF3Connection - executeICF3Transaction - ICF3Request: {}", icf3DTORequest);
        ICF3Response icf3Response = rbvdR051.executePolicyCancellation(icf3DTORequest);
        LOGGER.info("***** ICF3Connection - executeICF3Transaction - ICF3Response: {}", icf3Response);

        if (icf3Response.getHostAdviceCode() != null) {
            LOGGER.info("***** ICF3Connection - executeICF3Transaction - Error at icf3 execution - Host advice code: {}", icf3Response.getHostAdviceCode());
            this.addAdviceWithDescription("RBVD00000170", icf3Response.getHostMessage());
            return null;
        }
        LOGGER.info("***** ICF3Connection - executeICF3Transaction - End");
    ...
    public ICF3Request buildICF3Request(InputParametersPolicyCancellationDTO input, Map<String, Object> cancellationRequest, Map<String, Object> policy, ICF2Response icf2Response, String productCode, String authorizeReturnFlag){
        ...
        LOGGER.info("***** ICF3Connection - cancellationRequest: {}", cancellationRequest);
        ...
        LOGGER.info("***** ICF3Connection - icf3DTORequest: {}", icf3DTORequest);
    ...
    private void mapAmountsToBeReturned(Map<String, Object> cancellationRequest, ICF3Request icf3DTORequest) {
        LOGGER.info("***** ICF3Connection - mapAmountsToBeReturned() START - cancellationRequest: {}", cancellationRequest);
        if(cancellationRequest != null ) {
            if(cancellationRequest.get(RBVDProperties.FIELD_INSRC_COMPANY_RETURNED_AMOUNT.getValue()) != null){
                icf3DTORequest.setCOMRIMA(NumberUtils.toDouble(Objects.toString(cancellationRequest.get(RBVDProperties.FIELD_INSRC_COMPANY_RETURNED_AMOUNT.getValue()), "0.0")));
            }
            if(cancellationRequest.get(RBVDProperties.FIELD_PREMIUM_AMOUNT.getValue()) != null){
                icf3DTORequest.setMONTDEV(NumberUtils.toDouble(Objects.toString(cancellationRequest.get(RBVDProperties.FIELD_PREMIUM_AMOUNT.getValue()), "0.0")));
            }
        }
    }
    ...
    public EntityOutPolicyCancellationDTO mapICF3Response(InputParametersPolicyCancellationDTO input, ICF3Response icf3Response, Map<String, Object> cancellationRequest){
        LOGGER.info("***** mapICF3Response - icf3Response: {}", icf3Response);
        ...
        CommonCancellationDTO commonCancellationDTO = new CommonCancellationDTO();
        commonCancellationDTO.setId("");
        ...
        contractCancellation.setContractType("");
        contractCancellation.setNumberType(commonCancellationDTO);
        contractCancellation.setProductType(commonCancellationDTO);
        contractCancellation.setNumber("");
        ...
        LOGGER.info("***** mapICF3Response - output: {}", output);
